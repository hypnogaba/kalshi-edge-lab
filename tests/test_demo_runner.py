...
# The tick the feed publishes for KXBTCPERP. The duel key counts the price in
# ticks, so a state built without one cannot pair the two bots' decisions at all.
TICK = 1.0


def fresh_state() -> DemoState:
    return DemoState(StrategyConfig(min_print_size=50, cooldown_ns=0), markout_ns=0,
                     tick_of=lambda _market: TICK)
...
def test_a_state_built_without_ticks_refuses_to_start():
    """Rather than pairing nothing and reporting two empty scoreboards, which
    reads as a quiet market instead of a broken one."""
    with pytest.raises(TypeError):
        DemoState(StrategyConfig(min_print_size=50, cooldown_ns=0), markout_ns=0)


def test_a_market_with_no_tick_yet_is_skipped_not_paired_on_a_guess():
    """Reference data arrives seconds after joining. Until it does, the price
    cannot be quantised, and a guessed key would pair the wrong prints."""
    state = DemoState(StrategyConfig(min_print_size=50, cooldown_ns=0), markout_ns=0,
                      tick_of=lambda _market: None)
    for event in dz_quote(100.0, 100.5, 0) + pub_quote(100.0, 100.5, 0):
        state.on_event(DZ if event.source is Source.DZ_FEED else PUBLIC, event)
    state.on_event(DZ, trade(Source.DZ_FEED, 1 * MS, 500_000))
    state.on_event(PUBLIC, trade(Source.MARGIN_WS, 8 * MS, 500_000))
    ...
    assert state.snapshot()["head_to_head"]["n"] == 0
